#!/bin/bash
echo "----------------------------------------------------------------------------"
echo " Automatically set up development environment for FreeRTOS-platform on ESP32P4"
echo "----------------------------------------------------------------------------"
echo ""
...
export forte_bin_dir="bin/esp32p4"
...
if [ -d "$forte_bin_dir" ]; then
  ...
  cmake -G "Ninja" \
    -DCMAKE_TOOLCHAIN_FILE="${FORTE_BUILDSUPPORT_DIRECTORY}/toolchains/toolchain-esp32p4.cmake" \
    ...
    -DCMAKE_INSTALL_PREFIX=../../../Application/lib/esp32p4 \
    -DCMAKE_CXX_STANDARD_INCLUDE_DIRECTORIES="${PWD}/../../../Application/build_ESP32P4_NodeISO20/config/" \
    ...
    -DFORTE_COM_MODBUS_LIB_DIR="../../../Application/components/libmodbus" \
    ...
    -DFORTE_MODULE_OSCAT_BASIC=ON \
    -DFORTE_MODULE_OSCAT_NETWORK=ON \
    -DFORTE_MODULE_OSCAT_BUILDING=ON \
    -DFORTE_MODULE_ADAPTER=ON \
    -DFORTE_MODULE_LOGIBUS_SIGNALPROCESSING=ON \
    -DFORTE_MODULE_FBOOT_VERSION=ON \
    ../../
